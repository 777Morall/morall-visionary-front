...
export const Route = createFileRoute("/historico")({
  head: () => ({ meta: [{ title: "Histórico — Morall Buscas" }] }),
  component: Historico,
});

const statuses = ["Todos", "Concluída", "Pendente", "Falhou"];
    ...
    () =>
      recentQueries.filter(
        (r) =>
          ...
          (q === "" || r.id.toLowerCase().includes(q.toLowerCase()) || r.type.toLowerCase().includes(q.toLowerCase())),
      ),
    [q, status],
  );
      ...
      <GlassCard className="overflow-hidden">
        <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-1 items-center gap-3 rounded-xl border border-border bg-bg-deep/50 px-3.5 sm:max-w-xs">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar consulta..." className="h-11 w-full bg-transparent text-sm text-text-strong outline-none placeholder:text-text-dim" />
          </div>
            ...
            <tbody>
              {rows.map((r, i) => (
                <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }} className="border-b border-border/50 hover:bg-bg-deep/30">
                  <td className="px-6 py-3.5 text-text-soft">{r.type}</td>
                  <td className="px-6 py-3.5 font-mono text-text-dim">{r.input}</td>
                  <td className="px-6 py-3.5 text-text-dim">{r.date}</td>
                  <td className="px-6 py-3.5 text-text-dim">{r.plan}</td>
        ...
        {rows.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
            ...
            <p className="text-sm text-text-dim">Nenhuma consulta encontrada com esses filtros.</p>
          </div>
        )}
      </GlassCard>
    </DashboardLayout>
